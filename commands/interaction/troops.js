...
const { troops, siege, builderTroops, superTroops, darkTroops } = require('../../data/coc/troops.json');
        ...
        for (let i = 0; i < accounts.length; i++) {
            ...
            response.troops.map(troop => {
                if (troops.includes(troop.name) && troop.maxLevel !== 18) {
                    troopStr += `${troop.name}: ${troop.level} | ${troop.maxLevel} \n`
                }
                if (darkTroops.includes(troop.name)) {
                    darkTroopStr += `${troop.name}: ${troop.level} | ${troop.maxLevel} \n`
                }
                if (superTroops.includes(troop.name)) {
                    superTroopStr += `${troop.name}: ${troop.level} | ${troop.maxLevel} \n`
                }
                if (siege.includes(troop.name)) {
                    siegeTroopStr += `${troop.name}: ${troop.level} | ${troop.maxLevel} \n`
                }
                if (builderTroops.includes(troop.name) && troop.maxLevel === 18) {
                    builderTroopStr += `${troop.name}: ${troop.level} | ${troop.maxLevel} \n`
                }
            })
            embed.addFields({
